...
const EMPTY_F: FeeForm = {
  studentName:'', feeMonth:'', feeType:'Monthly Tuition', amountDue:'', amountPaid:'',
  paymentMethod:'UPI', paymentStatus:'Paid', dueDate:'', paymentDate:'', reference:'', notes:'',
};
...
export function Fees() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setStudents(studentRows.slice(1).map(r => r[0]).filter(Boolean));
    } catch (e: any) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await appendRows(token, SHEET_ID, `'${TABS.FEES}'!A:N`, [[
        ...
        form.paymentMethod, form.paymentStatus, form.reference, form.notes,
      ]]);
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      const row = editTarget.rowIndex;
      const tab = TABS.FEES;
      const balance = parseFloat(form.amountDue) - parseFloat(form.amountPaid || '0');
      // Update non-formula columns (skip C=batch formula, H=balance formula)
      await batchWrite(token, SHEET_ID, [
        ...
        { range: `'${tab}'!N${row}`, value: form.notes },
      ]);
  ...
  return (
    <Layout title="Fees" action={
      ...
      <div className="p-4 space-y-3">
        ...
        {visible.length === 0 && (
          <div className="text-center py-10 text-gray-400">
            <p className="text-3xl mb-2">💰</p><p>No fee entries yet.</p>
          </div>
        )}
          ...
          <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500">{f.feeType}{f.feeMonth ? ` · ${f.feeMonth}` : ''}</p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { setEditTarget(f); setForm(feeToForm(f)); }}
                  className="text-gray-400 text-lg leading-none px-1">✏️</button>
              </div>
            </div>
            <div className="flex gap-4 mt-2 text-sm text-gray-600 flex-wrap">
              <span>Due: ₹{f.amountDue}</span>
              <span>Paid: <strong className="text-green-700">₹{f.amountPaid}</strong></span>
              {parseFloat(f.balance) > 0 && <span className="text-red-600">Bal: ₹{f.balance}</span>}
              {f.paymentMethod && <span className="text-gray-400 text-xs">{f.paymentMethod}</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Add modal */}
      {showAdd && (
        <FeeModal title="Add Payment" onClose={() => setShowAdd(false)}
          form={form} setForm={setForm} students={students}
          onSave={handleAdd} saving={saving} disabled={!form.studentName || !form.amountDue} />
      )}

      {/* Edit modal */}
      {editTarget && (
        <FeeModal title="Edit Payment" onClose={() => setEditTarget(null)}
          form={form} setForm={setForm} students={students}
          onSave={handleEdit} saving={saving} disabled={!form.studentName} />
      )}
    </Layout>
  );
}

function FeeModal({ title, onClose, form, setForm, students, onSave, saving, disabled }:
  { title:string; onClose:()=>void; form:FeeForm; setForm:(f:FeeForm)=>void;
    students:string[]; onSave:()=>void; saving:boolean; disabled:boolean }) {
  const u = (k: keyof FeeForm) =>
    (e: React.ChangeEvent<HTMLInputElement|HTMLSelectElement>) => setForm({ ...form, [k]: e.target.value });
  return (
    <div className="fixed inset-0 bg-black/50 flex items-end z-50" onClick={onClose}>
      <div className="bg-white w-full rounded-t-2xl p-5 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <F label="Student *">
            <select value={form.studentName} onChange={u('studentName')} className="input">
          ...
          <F label="Fee Type">
            <select value={form.feeType} onChange={u('feeType')} className="input">
              {['Monthly Tuition','Admission','Tournament','Van','Materials','Other'].map(o => <option key={o}>{o}</option>)}
            </select>
          </F>
          ...
          <F label="Payment Method">
            <select value={form.paymentMethod} onChange={u('paymentMethod')} className="input">
              {['Cash','UPI','Bank Transfer','Card','Cheque'].map(o => <option key={o}>{o}</option>)}
            </select>
          </F>
          <F label="Payment Status">
            <select value={form.paymentStatus} onChange={u('paymentStatus')} className="input">
              {['Paid','Partial','Pending','Overdue','Waived'].map(o => <option key={o}>{o}</option>)}
            </select>
          </F>
          <div className="grid grid-cols-2 gap-2">
          ...
          <F label="Notes"><input value={form.notes} onChange={u('notes')} className="input" /></F>
        </div>
